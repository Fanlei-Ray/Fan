...
# ============================================================
# Right arm wrist-site fixed-pick debug
#
# 目标：
#   右臂先在固定点把 cube 抓起来。
#
# 核心区别：
#   不使用 right_gripper_tcp
#   使用 right_ee_control_point + 大 offset
#
# 正常基准仍然是：
...
RIGHT_SITE_NAME = "right_ee_control_point"
...
#
# 这些 offset 的意思是：
#   right_ee_control_point 目标位置 = cube_pos + grasp_offset
#
# 因为 right_ee_control_point 在腕部附近，所以 offset 比 TCP offset 大很多。
# ============================================================

EE_GRASP_OFFSETS = [
    # 先扫最可能的一批：右臂从 cube 右/后侧接近
    ("ee_x_m075_y_m060_z_m010", np.array([-0.075, -0.060, -0.010])),
    ("ee_x_m075_y_m045_z_m010", np.array([-0.075, -0.045, -0.010])),
    ("ee_x_m075_y_m030_z_m010", np.array([-0.075, -0.030, -0.010])),
    ("ee_x_m075_y_m015_z_m010", np.array([-0.075, -0.015, -0.010])),
    ("ee_x_m075_y_000_z_m010",  np.array([-0.075,  0.000, -0.010])),

    ("ee_x_m075_y_m060_z_000", np.array([-0.075, -0.060, 0.000])),
    ("ee_x_m075_y_m045_z_000", np.array([-0.075, -0.045, 0.000])),
    ("ee_x_m075_y_m030_z_000", np.array([-0.075, -0.030, 0.000])),
    ("ee_x_m075_y_m015_z_000", np.array([-0.075, -0.015, 0.000])),
    ("ee_x_m075_y_000_z_000",  np.array([-0.075,  0.000, 0.000])),

    # x 稍微变一点
    ("ee_x_m065_y_m045_z_m010", np.array([-0.065, -0.045, -0.010])),
    ("ee_x_m085_y_m045_z_m010", np.array([-0.085, -0.045, -0.010])),
    ("ee_x_m065_y_m030_z_m010", np.array([-0.065, -0.030, -0.010])),
    ("ee_x_m085_y_m030_z_m010", np.array([-0.085, -0.030, -0.010])),

    # z 再上下扫一点
    ("ee_x_m075_y_m045_z_p010", np.array([-0.075, -0.045, +0.010])),
    ("ee_x_m075_y_m045_z_m020", np.array([-0.075, -0.045, -0.020])),
    ("ee_x_m075_y_m030_z_p010", np.array([-0.075, -0.030, +0.010])),
    ("ee_x_m075_y_m030_z_m020", np.array([-0.075, -0.030, -0.020])),

    # y 正方向也扫一点，防止镜像方向判断错
    ("ee_x_m075_y_p015_z_m010", np.array([-0.075, +0.015, -0.010])),
    ("ee_x_m075_y_p030_z_m010", np.array([-0.075, +0.030, -0.010])),
    ("ee_x_m075_y_p045_z_m010", np.array([-0.075, +0.045, -0.010])),
]
...
POSTURE_CONFIGS = [
    ...
    ("j7_p030", {"openarm_right_joint7": +0.30}),
    ("j7_m030", {"openarm_right_joint7": -0.30}),
    ("j7_p060", {"openarm_right_joint7": +0.60}),
    ("j7_m060", {"openarm_right_joint7": -0.60}),
...
def solve_right_arm_ik(
    ...
    return final_error < 0.020, final_error, best_error, ctrl_targets
...
def ik_move_right_to(
    ...
    sim_steps(model, data, steps=140, viewer=viewer, realtime=realtime)
...
def run_trial(
    ...
    pregrasp_target = cube_pos + grasp_offset + np.array([0.0, 0.0, 0.110])
    ...
    final_site = get_site_pos(model, data, RIGHT_SITE_NAME)
...
def main():
    print("=" * 80)
    print("Right arm wrist-site fixed-pick debug")
    print("=" * 80)
    print("XML:", XML_PATH)
    ...
    if maybe_id(model, mujoco.mjtObj.mjOBJ_SITE, RIGHT_SITE_NAME) == -1:
        raise RuntimeError(
            f"找不到 {RIGHT_SITE_NAME}。请确认 XML 中存在 right_ee_control_point。"
        )

    results = []

    for offset_name, grasp_offset in EE_GRASP_OFFSETS:
    ...
    print("")
    print("=" * 80)
    print("TOP right wrist-site fixed-pick configs")
    print("=" * 80)
