...
"""OpenArm V13: hybrid FSM + Behavior Tree task-planning demo.
...
BT_TRACE_PATH = core.OUTPUT_DIR / "behavior_tree_trace_v13_hybrid.csv"
FSM_TRACE_PATH = core.OUTPUT_DIR / "fsm_trace_v13_hybrid.csv"
HYBRID_REPORT_PATH = core.OUTPUT_DIR / "hybrid_fsm_bt_report_v13.md"


def patch_v13_output_paths() -> None:
    """Route all outputs to V13-specific filenames so older successful runs are not overwritten."""
    core.LOG_PATH = core.OUTPUT_DIR / "task_log_v13_hybrid.csv"
    core.PLAN_PATH = core.OUTPUT_DIR / "execution_plan_v13_hybrid.csv"
    core.SUMMARY_PATH = core.OUTPUT_DIR / "task_summary_v13_hybrid.json"
    core.REPORT_PATH = core.OUTPUT_DIR / "presentation_report_v13_hybrid.md"
    core.STATE_MACHINE_PATH = core.OUTPUT_DIR / "state_machine_v13_hybrid.mmd"
    core.RUNBOOK_PATH = core.OUTPUT_DIR / "demo_runbook_v13_hybrid.txt"
    core.PATH_PLAN_PATH = core.OUTPUT_DIR / "path_plan_v13_hybrid.csv"
    core.COLLISION_LOG_PATH = core.OUTPUT_DIR / "collision_log_v13_hybrid.csv"
...
class HybridFSMBehaviorTreeRunner:
    ...
    def run(self, tasks: List[core.PickPlaceTask]) -> List[core.TaskResult]:
        ...
        print("\n" + "#" * 100)
        print("V13 Hybrid FSM + Behavior Tree Executor")
        print("#" * 100)
        print("执行方式：")
...
def write_hybrid_report(tasks: List[core.PickPlaceTask], results: List[core.TaskResult]) -> None:
    ...
    with open(HYBRID_REPORT_PATH, "w", encoding="utf-8") as f:
        f.write("# OpenArm V13 Hybrid FSM + Behavior Tree Demo\n\n")
        f.write("## Purpose\n\n")
        f.write("This demo combines a Behavior Tree task executor with a finite-state machine trace. ")
...
def main() -> None:
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        viewer.sync()
        print("viewer 已打开。2 秒后开始 V13 FSM+BT 混合任务规划 demo。")
        time.sleep(2.0)
