...
# ============================================================
# OpenArm bimanual task planner demo V7
#
# 目标：
...
#
# 推荐运行：
#   python scripts\openarm_bimanual_task_planner_demo_v7_json_tasks.py
#
# 快速无 viewer 测试：
#   python scripts\openarm_bimanual_task_planner_demo_v7_json_tasks.py --no-viewer
#
# 备用：只展示右臂稳定分支：
#   python scripts\openarm_bimanual_task_planner_demo_v7_json_tasks.py --right-only
# ============================================================
...
LOG_PATH = OUTPUT_DIR / "task_log_v7.csv"
PLAN_PATH = OUTPUT_DIR / "execution_plan_v7.csv"
SUMMARY_PATH = OUTPUT_DIR / "task_summary_v7.json"
REPORT_PATH = OUTPUT_DIR / "presentation_report_v7.md"
STATE_MACHINE_PATH = OUTPUT_DIR / "state_machine_v7.mmd"
RUNBOOK_PATH = OUTPUT_DIR / "demo_runbook_v7.txt"
PATH_PLAN_PATH = OUTPUT_DIR / "path_plan_v7.csv"
TASKS_JSON_EXAMPLE_PATH = OUTPUT_DIR / "tasks_v7_example.json"
...
class SafetyManager:
    ...
    def park_inactive_arm(self, active_arm: str, cube_pos: np.ndarray) -> SafetySnapshot:
        ...
        # 稳定版避让策略：非工作臂夹爪打开，保持 home/安全等待位。
        # 真正的 arm trajectory 避障后续可以接 RRT/MoveIt/轨迹优化；这里先做状态机级互斥与 park。
        if inactive == "left":
...
class PathPlanner:
    ...
    V6 的重点是把“直接调用 pick_and_place”升级为：
...
class BimanualTaskPlanner:
    ...
    def execute_one_task(self, task: PickPlaceTask) -> TaskResult:
        ...
        self.logger.log(task, state, selected_arm, safety=safety_snapshot, message="safety pre-check")
        ...
        self.logger.log(
            ...
            message=f"path planned, waypoints={len(waypoints)}, path_safe={path_safe}",
        )
        if not path_safe:
        ...
        self.logger.log(task, state, selected_arm, safety=safety_snapshot, message="inactive arm parked / safe")
...
def normalize_raw_task_dict(t: Dict[str, Any], index: int) -> Dict[str, Any]:
    """Normalize one task from JSON or built-in config into planner format."""
    if "cube_pos" not in t:
        raise ValueError(f"任务 #{index} 缺少 cube_pos 字段：{t}")

    cube_pos = np.asarray(t["cube_pos"], dtype=float)
    if cube_pos.shape != (3,):
        raise ValueError(f"任务 #{index} 的 cube_pos 必须是 3 个数，例如 [0.516, 0.0, 1.05]：{t}")

    return {
        "task_id": str(t.get("task_id", f"json_task_{index:03d}")),
        "object_name": str(t.get("object_name", "orange_cube")),
        "target_name": str(t.get("target_name", "black_frame")),
        "cube_pos": cube_pos,
        "description": str(t.get("description", "JSON task")),
    }
...
def load_tasks_from_json(path: Path) -> List[Dict[str, Any]]:
    ...
    """
    if not path.exists():
        raise FileNotFoundError(f"找不到任务 JSON 文件：{path}")
    ...
    if isinstance(payload, dict):
        if "tasks" not in payload:
            raise ValueError("JSON 顶层是 dict 时，必须包含 tasks 字段。")
        raw_tasks = payload["tasks"]
    else:
        raw_tasks = payload

    if not isinstance(raw_tasks, list) or len(raw_tasks) == 0:
        raise ValueError("任务 JSON 必须包含非空 tasks 列表。")

    return [normalize_raw_task_dict(t, i) for i, t in enumerate(raw_tasks, start=1)]


def write_example_tasks_json(path: Path = TASKS_JSON_EXAMPLE_PATH) -> None:
    """Write an editable JSON task queue for V7."""
    ensure_output_dir()
    example_payload = {
        "version": "v7_json_tasks",
        "selector_rule": f"right if cube_y >= {SELECT_RIGHT_IF_CUBE_Y_GE:.3f}, else left",
        "tasks": [
            {
                "task_id": "json_left_001",
                "object_name": "orange_cube",
                "target_name": "black_frame",
                "cube_pos": [0.516, 0.000, 1.050],
                "description": "JSON 任务 1：左臂执行",
            },
            {
                "task_id": "json_right_001",
                "object_name": "orange_cube",
                "target_name": "black_frame",
                "cube_pos": [0.516, 0.050, 1.050],
                "description": "JSON 任务 2：右臂执行",
            },
            {
                "task_id": "json_left_002",
                "object_name": "orange_cube",
                "target_name": "black_frame",
                "cube_pos": [0.516, 0.000, 1.050],
                "description": "JSON 任务 3：左臂再次执行",
            },
            {
                "task_id": "json_right_002",
                "object_name": "orange_cube",
                "target_name": "black_frame",
                "cube_pos": [0.516, 0.050, 1.050],
                "description": "JSON 任务 4：右臂再次执行",
            },
        ],
    }
    with open(path, "w", encoding="utf-8") as f:
        json.dump(example_payload, f, ensure_ascii=False, indent=2)
    print("[V7] 已生成可编辑任务 JSON：", path)
...
def print_presentation_header(tasks: List[PickPlaceTask]) -> None:
    print("=" * 100)
    print("OpenArm 双臂上层任务规划 Demo V7")
    print("=" * 100)
    print("核心演示：")
    ...
    print("  6. V6 额外增加 PathPlanner：生成 waypoint path、做 path safety check、导出 path_plan_v7.csv。")
    print("")
    print("State machine:")
    print("  INIT -> PERCEIVE -> SELECT_ARM -> SAFETY_CHECK -> PARK_INACTIVE_ARM")
    print("       -> PLAN_PATH -> PARK_INACTIVE_ARM -> EXECUTE_PICK_PLACE -> [RETRY_PICK_PLACE] -> [REPLAN_ARM] -> VERIFY -> DONE/FAILED")
    print("")
    ...
    print("TASKS_JSON_EXAMPLE_PATH:", TASKS_JSON_EXAMPLE_PATH)
...
"""
    ...
    runbook = f"""OpenArm 双臂任务规划 Demo V7 运行手册
...
恢复逻辑演示命令：
  python scripts\\openarm_bimanual_task_planner_demo_v7_json_tasks.py --simulate-recovery

队列演示命令：
  python scripts\\openarm_bimanual_task_planner_demo_v7_json_tasks.py --queue-demo
...
"""
    ...
    report = f"""# OpenArm 双臂上层任务规划系统 Demo V7 汇报记录
...
- **任务层**：`PickPlaceTask(object=orange_cube, target=black_frame)`，支持内置任务队列或外部 JSON 任务文件
...
V7 会为每个任务生成 waypoint-based path plan，并支持从 JSON 文件读取任务队列，保存到：`{PATH_PLAN_PATH}`。每个 waypoint 包含目标位置、motion_type、safety_role、expected_state 和 safe 标记。
...
"""
    ...
    print("[V7] 已生成汇报材料：")
    print("[V7] REPORT_PATH:", REPORT_PATH)
    print("[V7] STATE_MACHINE_PATH:", STATE_MACHINE_PATH)
    print("[V7] RUNBOOK_PATH:", RUNBOOK_PATH)
...
def parse_args():
    parser = argparse.ArgumentParser(description="OpenArm bimanual task planner demo V7 with JSON task input and waypoint path planning")
    parser.add_argument("--no-viewer", action="store_true", help="不打开 viewer，快速 headless 测试")
    parser.add_argument("--tasks-json", type=str, default="", help="从外部 JSON 文件读取任务队列，例如 outputs\bimanual_task_planner_demo\tasks_v7_example.json")
    parser.add_argument("--write-example-tasks", action="store_true", help="生成一个可编辑的 JSON 任务队列示例，然后退出")
    ...
    parser.add_argument("--dry-run-plan", action="store_true", help="只生成 execution_plan_v7.csv，不执行机械臂动作")
...
def main() -> None:
    ...
    if args.write_example_tasks:
        write_example_tasks_json(TASKS_JSON_EXAMPLE_PATH)
        print("下一步可运行：")
        print(f"python scripts\\openarm_bimanual_task_planner_demo_v7_json_tasks.py --tasks-json {TASKS_JSON_EXAMPLE_PATH}")
        return

    if args.tasks_json:
        raw_tasks = load_tasks_from_json(Path(args.tasks_json))
        print("[V7] 从 JSON 读取任务队列：", Path(args.tasks_json))
