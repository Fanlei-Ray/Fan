...
# ============================================================
# OpenArm bimanual task planner demo V6
#
# 目标：
...
#
# 推荐运行：
#   python scripts\openarm_bimanual_task_planner_demo_v6_path_planning.py
#
# 快速无 viewer 测试：
#   python scripts\openarm_bimanual_task_planner_demo_v6_path_planning.py --no-viewer
#
# 备用：只展示右臂稳定分支：
#   python scripts\openarm_bimanual_task_planner_demo_v6_path_planning.py --right-only
# ============================================================
...
LOG_PATH = OUTPUT_DIR / "task_log_v6.csv"
PLAN_PATH = OUTPUT_DIR / "execution_plan_v6.csv"
SUMMARY_PATH = OUTPUT_DIR / "task_summary_v6.json"
REPORT_PATH = OUTPUT_DIR / "presentation_report_v6.md"
STATE_MACHINE_PATH = OUTPUT_DIR / "state_machine_v6.mmd"
RUNBOOK_PATH = OUTPUT_DIR / "demo_runbook_v6.txt"
PATH_PLAN_PATH = OUTPUT_DIR / "path_plan_v6.csv"
...
def print_presentation_header(tasks: List[PickPlaceTask]) -> None:
    print("=" * 100)
    print("OpenArm 双臂上层任务规划 Demo V6")
    print("=" * 100)
    print("核心演示：")
    ...
    print("  6. V6 额外增加 PathPlanner：生成 waypoint path、做 path safety check、导出 path_plan_v6.csv。")
    print("")
    print("State machine:")
...
"""
    ...
    runbook = f"""OpenArm 双臂任务规划 Demo V6 运行手册
...
恢复逻辑演示命令：
  python scripts\\openarm_bimanual_task_planner_demo_v6_path_planning.py --simulate-recovery

队列演示命令：
  python scripts\\openarm_bimanual_task_planner_demo_v6_path_planning.py --queue-demo
...
"""
    ...
    report = f"""# OpenArm 双臂上层任务规划系统 Demo V6 汇报记录
...
V6 会为每个任务生成 waypoint-based path plan，保存到：`{PATH_PLAN_PATH}`。每个 waypoint 包含目标位置、motion_type、safety_role、expected_state 和 safe 标记。
...
"""
    ...
    print("[V6] 已生成汇报材料：")
    print("[V6] REPORT_PATH:", REPORT_PATH)
    print("[V6] STATE_MACHINE_PATH:", STATE_MACHINE_PATH)
    print("[V6] RUNBOOK_PATH:", RUNBOOK_PATH)
...
def parse_args():
    parser = argparse.ArgumentParser(description="OpenArm bimanual task planner demo V6 with waypoint path planning")
    ...
    parser.add_argument("--dry-run-plan", action="store_true", help="只生成 execution_plan_v6.csv，不执行机械臂动作")
