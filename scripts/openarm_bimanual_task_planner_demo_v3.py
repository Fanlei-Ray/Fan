...
# ============================================================
# OpenArm bimanual task planner demo V3
#
# 目标：
...
#
# 推荐运行：
#   python scripts\openarm_bimanual_task_planner_demo_v3.py
#
# 快速无 viewer 测试：
#   python scripts\openarm_bimanual_task_planner_demo_v3.py --no-viewer
#
# 备用：只展示右臂稳定分支：
#   python scripts\openarm_bimanual_task_planner_demo_v3.py --right-only
# ============================================================
...
LOG_PATH = OUTPUT_DIR / "task_log_v3.csv"
...
class BimanualTaskPlanner:
    ...
    def execute_with_recovery(
        ...
        V3 的重点不是为了让成功 case 更复杂，而是让上层 planner 具备恢复能力：
        - 第一次失败：同一只手自动 retry；
...
def print_presentation_header(tasks: List[PickPlaceTask]) -> None:
    print("=" * 100)
    print("OpenArm 双臂上层任务规划 Demo V3")
    print("=" * 100)
    print("核心演示：")
...
def parse_args():
    parser = argparse.ArgumentParser(description="OpenArm bimanual task planner demo V3")
