...
# ============================================================
# OpenArm bimanual task planner demo V8
#
# 目标：
...
#
# 推荐运行：
#   python scripts\openarm_bimanual_task_planner_demo_v8_collision_detection.py
#
# 快速无 viewer 测试：
#   python scripts\openarm_bimanual_task_planner_demo_v8_collision_detection.py --no-viewer
#
# 备用：只展示右臂稳定分支：
#   python scripts\openarm_bimanual_task_planner_demo_v8_collision_detection.py --right-only
# ============================================================
...
LOG_PATH = OUTPUT_DIR / "task_log_v8.csv"
PLAN_PATH = OUTPUT_DIR / "execution_plan_v8.csv"
SUMMARY_PATH = OUTPUT_DIR / "task_summary_v8.json"
REPORT_PATH = OUTPUT_DIR / "presentation_report_v8.md"
STATE_MACHINE_PATH = OUTPUT_DIR / "state_machine_v8.mmd"
RUNBOOK_PATH = OUTPUT_DIR / "demo_runbook_v8.txt"
PATH_PLAN_PATH = OUTPUT_DIR / "path_plan_v8.csv"
COLLISION_LOG_PATH = OUTPUT_DIR / "collision_log_v8.csv"
...
class CollisionMonitor:
    ...
    说明：
      ...
      - left-right 机械臂互碰、机械臂撞 table/floor/world 等会标成 dangerous。
    """
    ...
    def is_dangerous(self, category: str, dist: float) -> bool:
        ...
        if category in {"ROBOT_ROBOT", "ROBOT_ENV", "ROBOT_OTHER"}:
            return True
        return False
    ...
    def snapshot(self, task_id: str, state: str, selected_arm: str, message: str = "") -> Dict[str, Any]:
        ...
        print(
            ...
            f"min_contact_dist={min_contact_dist}"
        )
        if dangerous_count > 0:
            print("[COLLISION_CHECK] warning: dangerous contact detected, see collision_log_v8.csv")
...
class PathPlanner:
    ...
    V8 的重点是把“直接调用 pick_and_place”升级为：
...
def print_presentation_header(tasks: List[PickPlaceTask]) -> None:
    print("=" * 100)
    print("OpenArm 双臂上层任务规划 Demo V8")
    print("=" * 100)
    print("核心演示：")
    ...
    print("  6. V8 增加 MuJoCo contact 碰撞检测日志 collision_log_v8.csv。")
    print("  7. V6/V8 包含 PathPlanner：生成 waypoint path、做 path safety check、导出 path_plan_v8.csv。")
    print("")
    print("State machine:")
...
"""
    ...
    runbook = f"""OpenArm 双臂任务规划 Demo V8 运行手册
...
恢复逻辑演示命令：
  python scripts\\openarm_bimanual_task_planner_demo_v8_collision_detection.py --simulate-recovery

队列演示命令：
  python scripts\\openarm_bimanual_task_planner_demo_v8_collision_detection.py --queue-demo
...
"""
    ...
    report = f"""# OpenArm 双臂上层任务规划系统 Demo V8 汇报记录
...
V8 会为每个任务生成 waypoint-based path plan，保存到：`{PATH_PLAN_PATH}`。每个 waypoint 包含目标位置、motion_type、safety_role、expected_state 和 safe 标记。
...
def parse_args():
    parser = argparse.ArgumentParser(description="OpenArm bimanual task planner demo V8 with waypoint path planning")
    ...
    parser.add_argument("--dry-run-plan", action="store_true", help="只生成 execution_plan_v8.csv，不执行机械臂动作")
