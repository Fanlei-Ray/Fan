...
# ============================================================
# OpenArm bimanual task planner demo V9
#
# 目标：
...
#
# 推荐运行：
#   python scripts\openarm_bimanual_task_planner_demo_v9_collision_detection.py
#
# 快速无 viewer 测试：
#   python scripts\openarm_bimanual_task_planner_demo_v9_collision_detection.py --no-viewer
#
# 备用：只展示右臂稳定分支：
#   python scripts\openarm_bimanual_task_planner_demo_v9_collision_detection.py --right-only
# ============================================================
...
LOG_PATH = OUTPUT_DIR / "task_log_v9.csv"
PLAN_PATH = OUTPUT_DIR / "execution_plan_v9.csv"
SUMMARY_PATH = OUTPUT_DIR / "task_summary_v9.json"
REPORT_PATH = OUTPUT_DIR / "presentation_report_v9.md"
STATE_MACHINE_PATH = OUTPUT_DIR / "state_machine_v9.mmd"
RUNBOOK_PATH = OUTPUT_DIR / "demo_runbook_v9.txt"
PATH_PLAN_PATH = OUTPUT_DIR / "path_plan_v9.csv"
COLLISION_LOG_PATH = OUTPUT_DIR / "collision_log_v9.csv"
...
class CollisionMonitor:
    ...
    说明：
      ...
      - V9 修正了 V8 的误报问题：未识别的 ROBOT_OTHER 只记录为 unknown，不再直接算 dangerous。
      - 只有明确的 left-right 互碰、机械臂撞 table/floor/ground/plane 才会标成 dangerous。
    """
    ...
    def classify(self, geom1: str, geom2: str, body1: str, body2: str) -> str:
        ...
        task_tokens = ["orange", "cube", "black", "frame"]
    ...
    def is_dangerous(self, category: str, dist: float) -> bool:
        ...
        # V8 把 ROBOT_OTHER 也算 dangerous，容易把模型里的未命名/未知接触误报为危险。
        # V9 只把“明确危险”的类别算 dangerous：左右臂互碰、机械臂撞环境。
        if category in {"ROBOT_ROBOT", "ROBOT_ENV"}:
            return True
        return False
    ...
    def snapshot(self, task_id: str, state: str, selected_arm: str, message: str = "") -> Dict[str, Any]:
        ...
        if dangerous_count > 0:
            print("[COLLISION_CHECK] warning: dangerous contact detected, see collision_log_v9.csv")
...
class PathPlanner:
    ...
    V9 的重点是把“直接调用 pick_and_place”升级为：
...
def print_presentation_header(tasks: List[PickPlaceTask]) -> None:
    print("=" * 100)
    print("OpenArm 双臂上层任务规划 Demo V9 Collision Filter")
    print("=" * 100)
    print("核心演示：")
    ...
    print("  6. V9 增加 MuJoCo contact 碰撞检测日志 collision_log_v9.csv。")
    print("  7. V6/V9 包含 PathPlanner：生成 waypoint path、做 path safety check、导出 path_plan_v9.csv。")
    print("")
    print("State machine:")
...
"""
    ...
    runbook = f"""OpenArm 双臂任务规划 Demo V9 运行手册
...
恢复逻辑演示命令：
  python scripts\\openarm_bimanual_task_planner_demo_v9_collision_detection.py --simulate-recovery

队列演示命令：
  python scripts\\openarm_bimanual_task_planner_demo_v9_collision_detection.py --queue-demo
...
"""
    ...
    report = f"""# OpenArm 双臂上层任务规划系统 Demo V9 汇报记录
...
V9 会为每个任务生成 waypoint-based path plan，保存到：`{PATH_PLAN_PATH}`。每个 waypoint 包含目标位置、motion_type、safety_role、expected_state 和 safe 标记。
...
"""
    ...
    print("[V9] 已生成汇报材料：")
    print("[V9] REPORT_PATH:", REPORT_PATH)
    print("[V9] STATE_MACHINE_PATH:", STATE_MACHINE_PATH)
    print("[V9] RUNBOOK_PATH:", RUNBOOK_PATH)
...
def parse_args():
    parser = argparse.ArgumentParser(description="OpenArm bimanual task planner demo V9 with waypoint path planning")
    ...
    parser.add_argument("--dry-run-plan", action="store_true", help="只生成 execution_plan_v9.csv，不执行机械臂动作")
