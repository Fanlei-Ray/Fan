...
# ============================================================
# OpenArm bimanual task planner demo V5
#
# 目标：
...
#
# 推荐运行：
#   python scripts\openarm_bimanual_task_planner_demo_v5.py
#
# 快速无 viewer 测试：
#   python scripts\openarm_bimanual_task_planner_demo_v5.py --no-viewer
#
# 备用：只展示右臂稳定分支：
#   python scripts\openarm_bimanual_task_planner_demo_v5.py --right-only
# ============================================================
...
LOG_PATH = OUTPUT_DIR / "task_log_v5.csv"
PLAN_PATH = OUTPUT_DIR / "execution_plan_v5.csv"
SUMMARY_PATH = OUTPUT_DIR / "task_summary_v5.json"
REPORT_PATH = OUTPUT_DIR / "presentation_report_v5.md"
STATE_MACHINE_PATH = OUTPUT_DIR / "state_machine_v5.mmd"
RUNBOOK_PATH = OUTPUT_DIR / "demo_runbook_v5.txt"
    ...
    with open(PLAN_PATH, "w", newline="", encoding="utf-8") as f:
        ...
        for i, task in enumerate(tasks, start=1):
            ...
            writer.writerow([
                ...
                arm,
                inactive,
                "INIT>PERCEIVE>SELECT_ARM>SAFETY_CHECK>PARK_INACTIVE_ARM>EXECUTE_PICK_PLACE>VERIFY>DONE",
                task.description,
            ])
...
def write_summary_json(results: List[TaskResult]) -> None:
    ensure_output_dir()
    payload = {
        "demo": "openarm_bimanual_task_planner_demo_v5",
...
def print_presentation_header(tasks: List[PickPlaceTask]) -> None:
    print("=" * 100)
    print("OpenArm 双臂上层任务规划 Demo V5")
    print("=" * 100)
    print("核心演示：")
    ...
    print("  6. V5 额外生成汇报材料：report Markdown、状态机 Mermaid、runbook。")
    print("")
    print("State machine:")
    print("  INIT -> PERCEIVE -> SELECT_ARM -> SAFETY_CHECK -> PARK_INACTIVE_ARM")
    print("       -> EXECUTE_PICK_PLACE -> [RETRY_PICK_PLACE] -> [REPLAN_ARM] -> VERIFY -> DONE/FAILED")
    print("")
...
def write_presentation_assets(tasks: List[PickPlaceTask], results: List[TaskResult]) -> None:
    ...
    SAFETY_CHECK --> PARK_INACTIVE_ARM: workspace_ok
...
"""
    ...
    runbook = f"""OpenArm 双臂任务规划 Demo V5 运行手册
...
恢复逻辑演示命令：
  python scripts\\openarm_bimanual_task_planner_demo_v5.py --simulate-recovery

队列演示命令：
  python scripts\\openarm_bimanual_task_planner_demo_v5.py --queue-demo
...
"""
    ...
    report = f"""# OpenArm 双臂上层任务规划系统 Demo V5 汇报记录
...
"""
    ...
    print("[V5] 已生成汇报材料：")
    print("[V5] REPORT_PATH:", REPORT_PATH)
    print("[V5] STATE_MACHINE_PATH:", STATE_MACHINE_PATH)
    print("[V5] RUNBOOK_PATH:", RUNBOOK_PATH)
...
def parse_args():
    parser = argparse.ArgumentParser(description="OpenArm bimanual task planner demo V5")
    ...
    parser.add_argument("--dry-run-plan", action="store_true", help="只生成 execution_plan_v5.csv，不执行机械臂动作")
