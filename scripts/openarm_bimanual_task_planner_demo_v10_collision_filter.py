...
# ============================================================
# OpenArm bimanual task planner demo V10
#
# 目标：
...
#
# 推荐运行：
#   python scripts\openarm_bimanual_task_planner_demo_v10_collision_detection.py
#
# 快速无 viewer 测试：
#   python scripts\openarm_bimanual_task_planner_demo_v10_collision_detection.py --no-viewer
#
# 备用：只展示右臂稳定分支：
#   python scripts\openarm_bimanual_task_planner_demo_v10_collision_detection.py --right-only
# ============================================================
...
LOG_PATH = OUTPUT_DIR / "task_log_v10.csv"
PLAN_PATH = OUTPUT_DIR / "execution_plan_v10.csv"
SUMMARY_PATH = OUTPUT_DIR / "task_summary_v10.json"
REPORT_PATH = OUTPUT_DIR / "presentation_report_v10.md"
STATE_MACHINE_PATH = OUTPUT_DIR / "state_machine_v10.mmd"
RUNBOOK_PATH = OUTPUT_DIR / "demo_runbook_v10.txt"
PATH_PLAN_PATH = OUTPUT_DIR / "path_plan_v10.csv"
COLLISION_LOG_PATH = OUTPUT_DIR / "collision_log_v10.csv"
...
class CollisionMonitor:
    ...
    说明：
      ...
      - V10 修正了 V8 的误报问题：未识别的 ROBOT_OTHER 只记录为 unknown，不再直接算 dangerous。
      - V10 过滤已知 cell_table_col 与 link4/link5 的模型静态接触，并忽略 2mm 以下数值级轻微穿透。
    """
    ...
    def snapshot(self, task_id: str, state: str, selected_arm: str, message: str = "") -> Dict[str, Any]:
        ...
        if dangerous_count > 0:
            print("[COLLISION_CHECK] warning: dangerous contact detected, see collision_log_v10.csv")
...
class PathPlanner:
    ...
    V10 的重点是把“直接调用 pick_and_place”升级为：
...
def print_presentation_header(tasks: List[PickPlaceTask]) -> None:
    print("=" * 100)
    print("OpenArm 双臂上层任务规划 Demo V10 Collision Filter")
    print("=" * 100)
    print("核心演示：")
    ...
    print("  6. V10 增加 MuJoCo contact 碰撞检测日志 collision_log_v10.csv。")
    print("  7. V6/V10 包含 PathPlanner：生成 waypoint path、做 path safety check、导出 path_plan_v10.csv。")
    print("")
    print("State machine:")
...
"""
    ...
    runbook = f"""OpenArm 双臂任务规划 Demo V10 运行手册
...
恢复逻辑演示命令：
  python scripts\\openarm_bimanual_task_planner_demo_v10_collision_detection.py --simulate-recovery

队列演示命令：
  python scripts\\openarm_bimanual_task_planner_demo_v10_collision_detection.py --queue-demo
...
"""
    ...
    report = f"""# OpenArm 双臂上层任务规划系统 Demo V10 汇报记录
...
V10 会为每个任务生成 waypoint-based path plan，保存到：`{PATH_PLAN_PATH}`。每个 waypoint 包含目标位置、motion_type、safety_role、expected_state 和 safe 标记。
...
"""
    ...
    print("[V10] 已生成汇报材料：")
    print("[V10] REPORT_PATH:", REPORT_PATH)
    print("[V10] STATE_MACHINE_PATH:", STATE_MACHINE_PATH)
    print("[V10] RUNBOOK_PATH:", RUNBOOK_PATH)
...
def parse_args():
    parser = argparse.ArgumentParser(description="OpenArm bimanual task planner demo V10 with waypoint path planning")
    ...
    parser.add_argument("--dry-run-plan", action="store_true", help="只生成 execution_plan_v10.csv，不执行机械臂动作")
