...
# ============================================================
# OpenArm bimanual task planner demo V4
#
# 目标：
...
#
# 推荐运行：
#   python scripts\openarm_bimanual_task_planner_demo_v4.py
#
# 快速无 viewer 测试：
#   python scripts\openarm_bimanual_task_planner_demo_v4.py --no-viewer
#
# 备用：只展示右臂稳定分支：
#   python scripts\openarm_bimanual_task_planner_demo_v4.py --right-only
# ============================================================
...
LOG_PATH = OUTPUT_DIR / "task_log_v4.csv"
PLAN_PATH = OUTPUT_DIR / "execution_plan_v4.csv"
SUMMARY_PATH = OUTPUT_DIR / "task_summary_v4.json"
...
def write_summary_json(results: List[TaskResult]) -> None:
    ensure_output_dir()
    payload = {
        "demo": "openarm_bimanual_task_planner_demo_v4",
...
def print_presentation_header(tasks: List[PickPlaceTask]) -> None:
    print("=" * 100)
    print("OpenArm 双臂上层任务规划 Demo V4")
    print("=" * 100)
    print("核心演示：")
...
def parse_args():
    parser = argparse.ArgumentParser(description="OpenArm bimanual task planner demo V4")
    ...
    parser.add_argument("--dry-run-plan", action="store_true", help="只生成 execution_plan_v4.csv，不执行机械臂动作")
