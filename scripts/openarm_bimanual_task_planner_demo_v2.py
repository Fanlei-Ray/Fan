...
# ============================================================
# OpenArm bimanual task planner demo V2
#
# 目标：
...
#
# 推荐运行：
#   python scripts\openarm_bimanual_task_planner_demo_v2.py
#
# 快速无 viewer 测试：
#   python scripts\openarm_bimanual_task_planner_demo_v2.py --no-viewer
#
# 备用：只展示右臂稳定分支：
#   python scripts\openarm_bimanual_task_planner_demo_v2.py --right-only
# ============================================================
...
LOG_PATH = OUTPUT_DIR / "task_log_v2.csv"
...
class BimanualTaskPlanner:
    ...
    def execute_one_task(self, task: PickPlaceTask) -> TaskResult:
        ...
        self.logger.log(task, state, selected_arm, safety=safety_snapshot, message="start pick_and_place")
        result = self.actions.pick_and_place(selected_arm, task)
        safety_snapshot = self.safety.snapshot(selected_arm, obs["cube_pos"], message="post action safety snapshot")
        self.logger.log(task, state, selected_arm, result=result, safety=safety_snapshot, message="pick_and_place finished")
...
def print_presentation_header(tasks: List[PickPlaceTask]) -> None:
    print("=" * 100)
    print("OpenArm 双臂上层任务规划 Demo V2")
    print("=" * 100)
    print("核心演示：")
    ...
    print("")
    print("State machine:")
    print("  INIT -> PERCEIVE -> SELECT_ARM -> SAFETY_CHECK -> PARK_INACTIVE_ARM -> EXECUTE_PICK_PLACE -> VERIFY -> DONE/FAILED")
    print("")
...
def parse_args():
    parser = argparse.ArgumentParser(description="OpenArm bimanual task planner demo V2")
